...
class Tolak extends BaseController
{
  ...
  public function index()
  {
    ...
    $data['_uri_datatable'] = base_url("admin/pendaftar/tolak/datatable");
    $data['_uri_batal'] = base_url("admin/pendaftar/tolak/batal");
    $data['_title_header'] = "Ditolak";
  ...
  public function batal($pendaftar_santri_id)
  {
    ...
    if ($pendaftarSantriModel->update($pendaftar_santri_id, $data)) {
      $this->session->setFlashdata('success', 'Berhasil membatalkan penolakan santri');
      return redirect()->to(base_url("admin/pendaftar/proses"));
    } else {
      $this->session->setFlashdata('error', 'Gagal membatalkan penolakan santri');
  ...
  public function datatable()
  {
    ...
    if ($method == 'post') {
      $pendaftarSantriModel = new PendaftarSantriModel();
      $where = ['pendaftar_santri_status' => 2];
      $like = null;
