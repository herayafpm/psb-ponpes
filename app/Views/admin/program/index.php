...
<div class="">
  <div class="page-title">
    <div class="title_left w-100">
      <h3>Program <span class="text-secondary" style="font-size:0.6em">Pon.Pes Darussalam Dukuhwaluh Purwokerto</span></h3>
    </div>
  </div>
      ...
      <div class="x_panel">
        <div class="x_content">
          ...
          <div class="form-group row">
            <div class="col-md-4 mb-1"><input class="form-control form-control-sm" type="text" id="program_nama" placeholder="Program" /></div>
          </div>
...
<script>
  ...
  function hapus(id) {
    var conf = confirm("Yakin ingin menghapus program ini?")
    if (conf) {
  ...
  $(document).ready(function() {
      ...
      "columns": [{
        ...
        },
        {
          "render": function(data, type, row, meta) {
            var html = ''
            html += '<button type="button" class="btn btn-info btn-sm" onClick="edit(' + row.program_id + ')">Edit</button>'
            html += '<button type="button" class="btn btn-danger btn-sm" onClick="hapus(' + row.program_id + ')">Hapus</button>'
            return html
          }
        },
      ],
    ...
    $('#program_nama').on('keyup change clear', function() {
      var value = $(this).val();
      data.program_nama = value;
